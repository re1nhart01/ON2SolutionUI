...
static const char* current_packet = CH_PACKETS[0];
  ...
  public:
    ...
    void on_init() override
    {
      ...
      navigator->register_screen("/main", [navigator, uart = this->uart_handler](const NavigationParam& params) {
        return std::make_unique<MainScreen>(navigator.get(), MainScreenProps{.params = params, .uart = uart});
      });
      ...
      navigator->register_screen("/settings", [navigator, uart = this->uart_handler](const NavigationParam& params) {
        return std::make_unique<SettingsScreen>(navigator.get(), SettingsScreenProps{.params = params, .uart = uart});
      });

      navigator->register_screen("/errors", [navigator, uart = this->uart_handler](const NavigationParam& params) {
        return std::make_unique<ErrorsScreen>(navigator.get(), ErrorsScreenProps{.params = params, .uart = uart});
      });

      navigator->register_screen("/info", [navigator, uart = this->uart_handler](const NavigationParam& params) {
        return std::make_unique<InfoScreen>(navigator.get(), InfoScreenProps{.params = params, .uart = uart});
      });

      navigator->register_screen("/charts", [navigator, uart = this->uart_handler](const NavigationParam& params) {
        return std::make_unique<ChartsScreen>(navigator.get(), ChartsScreenProps{.params = params, .uart = uart});
      });
