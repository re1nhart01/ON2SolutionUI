...
namespace ON2Solutions {
  ...
  std::vector<ErrorModel> ErrorsScreen::generate_test_data_models() {
    std::vector<ErrorModel> items;

    // Датчики (Error = true)
    for (int i = 1; i <= 4; ++i) {
      items.push_back({ "Oxygen sensor " + std::to_string(i) + " disconnected", true });
    }

    // Резервні канали (Error = false/true чергується)
    for (int i = 1; i <= 21; ++i) {
      items.push_back({ "Reserved channel (" + std::to_string(i) + ")", (i % 2 != 0) });
    }
  ...
  $$View ErrorsScreen::render_body() {
    ...
    return $View(ViewProps::up()
                     ...
                     .set_children(children(
                         $Text(TextProps::up().value("Errors")),
                         $PaginatedList(
                             PaginatedListProps()
                                 .w(720)
                                 .h(355)
