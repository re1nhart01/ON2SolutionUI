...
namespace ON2Solutions {
  void MainScreen::on_focus()
  {
    ...
    this->uart_handler = std::make_unique<UartHandler>(UART_NUM_2, GPIO_NUM_43, GPIO_NUM_44, 9600, 16384);
  ...
  void MainScreen::add_uart_data_event()
  {
    ...
    this->uart_handler->add_event_listener(
        UartTypes::UartHandlerEvent{
            ...
            .delegate = [moto_lvgl](const UartTypes::UartCallbackResponse& uart_data) {
              ...
              auto args = new AsyncStructure{
                  .packet = uart_data.response.packet,
              ...
              lv_async_call(
                  [](void* arg) {
                    ...
                    if (p->reactive_moto_lvgl) {
                      p->reactive_moto_lvgl->set([](const int& prev) {
                        return prev + 1;
                      });
                    }
                    delete p;
...
#pragma region UI
  ...
  $$Button MainScreen::render_footer() const
  {
    return $Button(ButtonProps::up()
              ...
              std::string status_str = GetTextValueFromStatus(
                  GetStatusFromTextValue(value.operative_data.status.data()));
  ...
  $$View MainScreen::render_body() const
  {
    auto make_circle = [&](const std::string& ref_name, int index) {
      return $Meter(
          MeterProps::up()
            .watch<Dataset>(DatasetStore::getInstance(), "outputs", [index, ref_name](Meter* self, const Dataset& value) {
                        float val = (ref_name == "oxygen_level") ? value.operative_data.oxygen_levels[index]
                                                                 : value.operative_data.oxygen_speed[index];

                        self->set_state([val](MeterProps& props) {
                          props.value(val);
                        });
            })
            .label("%")
